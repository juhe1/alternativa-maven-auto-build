...
VERSION_OVERRIDE = {
	...
	"org.eclipse.equinox.http.jetty:1.1.0-v20080425": "1.1.100.v20100519",
	...
	"platform.server.libraries.javax.Transaction:1.0.0": "1.1.0",
	"platform.server.libraries.org.apache.commons.logging:[1.0.0,2.0.0)": "1.0.1",
	"platform.server.libraries.javax.Persistence:[1.0.0, 2.0.0)": "1.0.0",
	"platform.server.libraries.net.sf.Cglib:[1.0.0.0, 2.0.0.0)": "1.0.5.3",
	"platform.server.core.general.Protocol:[1.0.0.0,1.1.0.0)": "1.0.1.12",
	"platform.server.libraries.slf4j.Log4j12:[1.0.0,2.0.0)": "1.5.2",
	"platform.server.libraries.org.junit4:[4.0.0,5.0.0)": "4.3.1",
	"projects.tanks.clients.fp10.TanksFonts:1.0.4.0": "1.0.6.0",
	"platform.server.libraries.org.Objectweb:[1.0.0.0, 2.0.0.0)": "1.0.1.3",
	"platform.server.core.general.System:[1.0.0.0,2.0.0.0)": "1.0.7.0",
}
...
class FileHashManager:
	...
	def files_changed_in_directory(self, directory:str) -> bool:
		...
		hash_file_path = ".\\.hash_files\\" + directory.replace("\\", ".").replace("..", ".") + ".json"
...
def map_pom_dependencies(pom_info:PomInfo, pom_dir_by_pom_signature:Dict[str, str]):
	...
	def create_dependency_from_element(element:ET.Element) -> Tuple[str, str, str]:
		...
		version:None |str = try_read_element_text(element.find("version"))
		...
		if version is not None:
			version_clean:str = version.strip() # Strip whitespace chars from beginning and end of version.
	...
	pom_info_by_pom_signature:Dict[str, PomInfo] = {}
...
# If we have already checked a directory, we don't have to check it again
...
def compile_pom(pom_info:PomInfo) -> bool:
	...
	def print_compiling_message():
		print_line = "* Compiling pom: " + pom_info.signature + " *"
		print()
	...
	if not os.path.exists(compilation_work_dir):
...
def compile_pom_and_its_dependencies(pom_info:PomInfo) -> bool:
	"""
	...
	while stack:
		...
		for dependency in current_pom_info.dependencies:
			if dependency.signature in resolved_dependencies:
				continue

			if dependency.signature in missing_dependencies:
			...
			if dependency.is_3rd:
				if os.path.exists(dependency_local_repo_path):
					resolved_dependencies.append(dependency.signature)
					continue
				...
				if download_file_from_3rd_repos(dependency_pom_path) is None:
					...
					missing_dependencies.append(dependency.signature)
					unsolvable_dependencies = True
					continue
			...
			if dependency.path == "":
				...
				if os.path.exists(dependency_local_repo_path):
					color_print(Bcolors.WARNING, "Can't compile dependency for " + current_pom_info.signature + " because dependency " + dependency.signature + " source code is missing." + 
					" Using one from local repo.")
					resolved_dependencies.append(dependency.signature)
					continue

				color_print(Bcolors.FAIL, "Can't compile dependency for " + current_pom_info.signature + " because dependency " + dependency.signature + " source code is missing.")
				missing_dependencies.append(dependency.signature)
				unsolvable_dependencies = True
				continue
		...
		if unsolvable_dependencies:
			missing_dependencies.append(current_pom_info.signature)
		...
		resolved_dependencies.append(current_pom_info.signature)
...
def start_repository_server():
	...
	# Create and start the HTTP server with your custom request handler
...
pom_group_id:str = "projects.tanks.server"
pom_artifact_id:str = "Runner"
pom_version:str = "1.41.2.0"

pom_info:PomInfo = PomInfo(pom_group_id, pom_artifact_id, pom_version, "", False)
map_pom_dependencies(pom_info, pom_dir_by_pom_signature)
compile_pom_and_its_dependencies(pom_info)
